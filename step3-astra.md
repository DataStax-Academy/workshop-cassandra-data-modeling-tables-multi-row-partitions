...
✅ Create database `workshops` and keyspace `ks_multi_row_partitions` if they do not exist:
```
astra db create workshops -k ks_multi_row_partitions --if-not-exist --wait
```

This operation may take a bit longer when creating a new database or resuming an existing hibernated database.

✅ Verify that database `workshops` is `ACTIVE` and keyspace `ks_multi_row_partitions` exists:
```
astra db get workshops
```

✅ Start the CQL shell and connect to database `workshops` and keyspace `ks_multi_row_partitions`:
```
clear
astra db cqlsh workshops -k ks_multi_row_partitions
```
